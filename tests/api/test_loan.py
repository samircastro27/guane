...
def test_get_all(
    ...
    async def mock_get_all(skip, limit, payload):
        return [test_data.loan_response]

    monkeypatch.setattr(loan_service, "get_all", mock_get_all)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.get(
            f"{test_constants.SERVICE_URL}{test_constants.LOAN_PREFIX}"
        )
...
def test_post(
    ...
    monkeypatch.setattr(loan_service, "create", mock_create)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.post(
            f"{test_constants.SERVICE_URL}{test_constants.LOAN_PREFIX}",
            json=test_data.loan_post,
        )
...
def test_get_by_id(
    ...
    monkeypatch.setattr(loan_service, "get_by_id", mock_get_by_id)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.get(
            f"{test_constants.SERVICE_URL}{test_constants.LOAN_PREFIX}/1"
        )
        assert response.status_code == 200
        return None
...
def test_patch_by_id(
    ...
    monkeypatch.setattr(loan_service, "update", mock_update)
    monkeypatch.setattr(loan_service, "get_by_id", mock_get_by_id)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.patch(
            f"{test_constants.SERVICE_URL}{test_constants.LOAN_PREFIX}/1",
            json=test_data.loan_update,
        )
...
def test_delete_by_id(
    ...
    monkeypatch.setattr(loan_service, "delete", mock_delete)
    monkeypatch.setattr(loan_service, "get_by_id", mock_get_by_id)
