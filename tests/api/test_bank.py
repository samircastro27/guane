...
def test_get_all(
    ...
    async def mock_get_all(skip, limit):
        return [test_data.bank_response]

    monkeypatch.setattr(bank_service, "get_all", mock_get_all)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.get(
            f"{test_constants.SERVICE_URL}{test_constants.BANK_PREFIX}"
        )
...
def test_post(
    ...
    monkeypatch.setattr(bank_service, "create", mock_create)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.post(
            f"{test_constants.SERVICE_URL}{test_constants.BANK_PREFIX}",
            json=test_data.bank_post,
        )
...
def test_get_by_id(
    ...
    monkeypatch.setattr(bank_service, "get_by_id", mock_get_by_id)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.get(
            f"{test_constants.SERVICE_URL}{test_constants.BANK_PREFIX}/1"
        )
        assert response.status_code == 200
        return None
...
def test_patch_by_id(
    ...
    monkeypatch.setattr(bank_service, "update", mock_update)
    monkeypatch.setattr(bank_service, "get_by_id", mock_get_by_id)

    with fastapi_dep(app).override({deps.get_current_active_user: user_fake}):
        response = test_app.patch(
            f"{test_constants.SERVICE_URL}{test_constants.BANK_PREFIX}/1",
            json=test_data.bank_update,
        )
...
def test_delete_by_id(
    ...
    monkeypatch.setattr(bank_service, "delete", mock_delete)
    monkeypatch.setattr(bank_service, "get_by_id", mock_get_by_id)
